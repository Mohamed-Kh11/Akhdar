  ...
  return (
    <div className="flex flex-col items-center min-h-screen pt-[130px] mb-[50px] font-['Noto_Kufi_Arabic']">
      ...
      <p className="mt-4 max-w-[80%] mb-14 font-medium text-center text-green-700 text-lg">
        نسعى من خلال برامجنا البيئية إلى تعزيز الوعي بأهمية الحفاظ على البيئة،
        حيث نعمل على زراعة الأشجار، وتنظيف الشواطئ، وإعادة التدوير، ونشر ثقافة
        استخدام الطاقة النظيفة مثل الشمسية والرياح. كما نُشرك المجتمع في حملات
        توعوية تهدف إلى بناء مستقبل أخضر ومستدام للأجيال القادمة.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10 px-7 w-full max-w-[1200px]">
        {programs.map((program, index) => (
          <Link key={index} href={`/programs/${program.slug}`}>
            <div className="flex flex-col  rounded-3xl overflow-hidden shadow-md transition-transform duration-300 hover:scale-[1.02] cursor-pointer bg-green-700">
